...
// ===== THREE.js 3D háttér (világos) + fallback =====
let scene, camera, renderer, sphere, l1, l2;

function init3D(){
  if (!hasWebGL()){ console.warn('WebGL nem elérhető – fallback megy.'); return; }
  ...
  const mat = new THREE.MeshPhysicalMaterial({
    color: 0x9bd7ff, metalness: .15, roughness: .2, clearcoat: 1, clearcoatRoughness: .08,
    transmission: .6, thickness: .7
  });
  sphere = new THREE.Mesh(geo, mat);
  scene.add(sphere);

  scene.add(new THREE.AmbientLight(0xffffff, .7));
  l1 = new THREE.PointLight(0x66b2ff, .9); l1.position.set(2.2, 1.6, 2.2); scene.add(l1);
  l2 = new THREE.PointLight(0xffffff, .7); l2.position.set(-2.0, -1.4, 2.0); scene.add(l2);
...
// várjuk meg a DOM-ot és a THREE-t
...
// ===== Buborékok felúsztatása =====
const bubbles = document.querySelectorAll('.bubble');
// az első betöltéskor is mutasd:
...
const dict = {
  hu: {
    ...
    bubble:{
      app:"Applikációk – natív és cross-platform, gyors prototípus és stabil release.",
      ...
      game:"Játékfejlesztés – Unity/Unreal, demó → Google Play, monetizáció."
    }
  },
  en: {
    title:"Tamás Horváth",
    ...
    bubble:{
      app:"Apps – native & cross-platform, rapid prototypes and stable releases.",
      ...
      game:"Game dev – Unity/Unreal, demo → Google Play, monetization."
    }
  }
};
